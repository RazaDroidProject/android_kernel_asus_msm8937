...
int ini_get_key(char *filedata, char *section, char *key, char *value)
{
    ...
    for (i = 0; i < g_used_key_num; i++) {
        if (fts_strncmp(section, g_st_ini_file_data[i].pSectionName, g_st_ini_file_data[i].iSectionNameLen) != 0)
            continue;
...
int print_key_data(void)
{
    ...
    for (i = 0; i < MAX_KEY_NUM; i++) {
        ...
        FTS_TEST_DBG("pSectionName_%d:%s, pKeyName_%d:%s\n,pKeyValue_%d:%s", i, g_st_ini_file_data[i].pSectionName, i, g_st_ini_file_data[i].pKeyName, i, g_st_ini_file_data[i].pKeyValue);
...
//      return CFG_ERR_OPEN_FILE;
    ...
    while (1) {                 /*find section */
        ...
        memcpy(g_st_ini_file_data[g_used_key_num].pKeyName, buf1, g_st_ini_file_data[g_used_key_num].iKeyNameLen);
        ...
        memcpy(g_st_ini_file_data[g_used_key_num].pKeyValue, buf1 + iEqualSign + 1, g_st_ini_file_data[g_used_key_num].iKeyValueLen);
