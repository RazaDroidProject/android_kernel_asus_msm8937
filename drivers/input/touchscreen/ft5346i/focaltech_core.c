...
static int fts_input_dev_report_key_event(struct ts_event *event, struct fts_ts_data *data)
{
    ...
    if (data->pdata->have_key) {
        if ((1 == event->touch_point || 1 == event->point_num) && (event->au16_y[0] == data->pdata->key_y_coord)) {
            ...
            if (event->point_num == 0) {
                ...
                for (i = 0; i < data->pdata->key_number; i++) {
                    if (event->au16_x[0] > (data->pdata->key_x_coords[i] - FTS_KEY_WIDTH) && event->au16_x[0] < (data->pdata->key_x_coords[i] + FTS_KEY_WIDTH)) {
...
static void fts_report_value(struct fts_ts_data *data)
{
    ...
    FTS_DEBUG("point number: %d, touch point: %d", event->point_num, event->touch_point);
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
    ...
    if (pdata->have_key) {
        ...
        FTS_DEBUG("%d: (%d, %d, %d), [%d, %d, %d][%d]",
                  pdata->key_number, pdata->keys[0], pdata->keys[1], pdata->keys[2], pdata->key_x_coords[0], pdata->key_x_coords[1], pdata->key_x_coords[2], pdata->key_y_coord);
    }
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    int ret; //modified by hebiao at 20170825
    int err;

    printk("kernel_tp %s %d addr = 0x%x\n", __func__, __LINE__, client->addr);
    if (client->addr != 0x38) {
	client->addr = 0x38;
	printk("kernel_tp %s %d addr = 0x%x\n", __func__, __LINE__, client->addr);
    }
...
//hebiao@wind-mobi.com 20170825 begin
    ...
    printk("kernel_tp %s %d irq=%d\n", __func__, __LINE__, client->irq);
    err = request_threaded_irq(client->irq, NULL, fts_ts_interrupt, pdata->irq_gpio_flags | IRQF_ONESHOT | IRQF_TRIGGER_FALLING, client->dev.driver->name, data);
    if (err) {
