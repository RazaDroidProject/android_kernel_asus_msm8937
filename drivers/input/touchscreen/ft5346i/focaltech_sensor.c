...
int fts_sensor_read_data(struct fts_ts_data *data)
{
    ...
    if (fts_psensor_support_enabled() && data->psensor_pdata->tp_psensor_opened) {
        ...
        if (!ret) {
            ...
            input_report_abs(data->psensor_pdata->input_psensor_dev, ABS_DISTANCE, data->psensor_pdata->tp_psensor_data);
...
int fts_sensor_suspend(struct fts_ts_data *data)
{
    ...
    if (fts_psensor_support_enabled() && device_may_wakeup(&data->client->dev) && data->psensor_pdata->tp_psensor_opened) {
...
int fts_sensor_init(struct fts_ts_data *data)
{
    ...
    if (fts_psensor_support_enabled()) {
        ...
        psensor_pdata = devm_kzalloc(&data->client->dev, sizeof(struct fts_psensor_platform_data), GFP_KERNEL);
        if (!psensor_pdata) {
